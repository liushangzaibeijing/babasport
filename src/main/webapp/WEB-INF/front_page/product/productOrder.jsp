...
<form action="${pageContext.request.contextPath}/front/buyer/confirmOrder.shtml" method="post">
...
<div class="w ofc case">
	...
	<div class="box bg_white">
		<dl class="distr">
		    <!-- 获取用户信息 -->
		    <input type="hidden" name="buyerId" value="${buyer.username}"/>
		    <input type="hidden" name="skuIds" value="${skuIds}"/>
			<dd>${buyer.realName}&nbsp;&nbsp;&nbsp;${buyer.email} <span style="margin-left: 30px">${buyer.addr}</span></dd>
		</dl>
	</div>
	...
	<div class="box bg_white pb">
		...
		<tbody>
			 <c:forEach var="buyItem" items="${buyCart.buyItems}">
			    <tr>
					<td >${buyItem.sku.product.no}</td>
					<td align="center">
					    <img alt="${buyItem.sku.product.name}" src="${baseUrl}${buyItem.sku.skuImg}" width="100px">
					</td>
					<td><a target="block" href="javascript:void(0);"> ${buyItem.sku.product.name}--${buyItem.sku.color.name}--${buyItem.sku.size}</a></td>
					<td>￥${buyItem.sku.marketPrice}元</td>
					<td>x${buyItem.account}</td>
					<td>${buyItem.sku.stockInventory}</td>
			      </tr>
			 </c:forEach>
		</tbody>
		</table>
		...
		<div class="ofc">
			<div class="r">
				<dl class="total">
					...
					<dd><em class="l">运费：</em>￥<var>${buyCart.deliveFee}</var></dd>
