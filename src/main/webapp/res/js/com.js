...
function oSearchSuggest(searchFuc) {
	...
	var key = "";
	 ...
	 this.init = function() {
		input.bind('keyup', sendKeyWord); //为input设置keyup(按键松开) 进行搜索
		input.bind('blur', function() {   //为input设置blur(失焦事件)  隐藏搜索结果的显示列表
	...
	var sendKeyWord = function(event) {
		    //keycode 37 方向键  向左  ，  38 方向键  向上，39 方向键 向右， 40方向键 向下
			//键盘选择下拉项 
			if (suggestWrap.css('display') == 'block' && event.keyCode == 38 || event.keyCode == 40) {
				 // alert("调用");
				var current = suggestWrap.find('li.hover'); //获取当前选中的商品项（搜索出来的）
				if (event.keyCode == 38) { //向上
					if (current.length > 0) {
						//将当前选中的商品项移除显示效果 并获取其前面的商品项
						var prevLi = current.removeClass('hover').prev();
						if (prevLi.length > 0) {
							prevLi.addClass('hover');  //给前一个设置为选中
							input.val(prevLi.find('span').html());  //将商品的名称设置到input中
						}
					} else { //没有选中，则按下向上按键 选中最后一个
						var last = suggestWrap.find('li:last');
						last.addClass('hover');
						input.val(last.find('span').html());
					}
					...
					if (current.length > 0) {
						...
						if (nextLi.length > 0) {
							...
							input.val(nextLi.find('span').html());
						}
					} else {
						var first = suggestWrap.find('li:first');
						first.addClass('hover');
						input.val(first.find("span").html());
					}
				}
				//输入字符
				//对鼠标键的弹起不做监控
	...
	this.dataDisplay = function(data) {
		...
		//往搜索框下拉建议显示栏中添加条目并显示 
		var li;
        //创建一个文档片段。 比document.createDocument() 多次调用的效率高
		var tmpFrag = document.createDocumentFragment();
		suggestWrap.find('ul').html('');  //清空ul中的数据
		for (var i = 0; i < data.length; i++) {
			li = document.createElement('LI');
			li.setAttribute("style", "z-index:10;");
			li.innerHTML = data[i];
		...
		/*
		suggestWrap.find("li").each(function(index,data){
			$(this).click(function(){
				$(this).find("span").remove();
				alert("进入click");
				input.val(this.innerHTML);
...
//根据关键字获取商品并将进行显示(异步操作  )
/*
function getSearchProduct(){
	var keyword = $('#gover_search_key').val();
	//重置删选条件
	//直接调用productList.shtml(重置条件 需要和数据库进行交互)
	
	//使用js来取消筛选条件（需要很好的js代码）
	$.ajax(
	  		{ 
	  		 url:'../front/getproduct.shtml',
	  		 type:"POST",
	  		 contentType:"application/json",
	  		 data:keyword,
	  		 dataType:"json",
...
function showProductList(data){
 ...
 for(i in data){
	       //alert(data[i].name);
         ul.append('<li>'
     			//+'<a href="${pageContext.request.contextPath}/front/${product.id}/productDetail.shtml" title="'+data[i].keywords+'" target="_blank" class="pic"><img src="${baseUrl}'+data[i].imgUrl+'" alt="'+data[i].name+'" /></a>'
     			+'<a href="${pageContext.request.contextPath}/html/product/${product.id}.html?isLogin=${isLogin}&username=${username}" title="'+data[i].keywords+'" target="_blank" class="pic"><img src="${baseUrl}'+data[i].imgUrl+'" alt="'+data[i].name+'" /></a>'
     			+'<dl>'
     			+'<dt><a href="${pageContext.request.contextPath}/html/product/${product.id}?isLogin=${isLogin}&username=${username}" title="'+data[i].keywords+'" target="_blank">'+data[i].name+'</a></dt>'
     			+'<dd class="h40">'+data[i].keywords+'</dd>'
     			+'<dd class="orange">￥'+data[i].minPrice+' ~ ￥'+data[i].maxPrice+'</dd>'
     			+'<dd>北京有货</dd>'
 ...
 * 检查该对象是否为正数
 */
function checkPositiveNumer(theObj){
	     //正在使用 /^\d*$/
	     var reNum=/^\d*$/; 
	     var flag = reNum.test(theObj); //判断是否为正整数数
	     if(reNum.test(theObj)) {
	    	 return true; 
	     }else { 
			//负数或者文字均返回
			return false; 
